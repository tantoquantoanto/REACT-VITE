...
const BookDetails = () => {
  ...
  const handleUpdateComment = async (updatedComment) => {
    try {
      setIsCommentLoading(true);
      const response = await fetch(
        `https://striveschool-api.herokuapp.com/api/comments/${editingCommentId}`,
        {
          method: "PUT",
          headers: {
            Authorization:
                ...
                {isCommentLoading ? (
                  ...
                  singleBookComments.map((singleBookComment) => (
                    <ul className="list-unstyled" key={singleBookComment._id}>
                      <li>
                        <strong>Author:</strong> {singleBookComment.author}
                      </li>
                      <li>
                        <strong>Comment:</strong> {singleBookComment.comment}
                      </li>
                      <li>
